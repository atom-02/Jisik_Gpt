...
final_prompt = ChatPromptTemplate.from_messages([
    ("system", """
    당신은 글쓰기 도우미입니다.
    학생이 현재까지 쓴 글을 읽고, 최소1점에서 최고 5점까지 점수를 부여하세요.
...
st.set_page_config(page_title="피드백 도우미", layout="wide")
...
with col1 :
    ...
    mmid_feedback_btn=st.button("지금까지의 글 점검하기", use_container_width=True)
    container2=st.container(border=True)

    with container1 :

        name=st.text_input("자신의 이름을 입력헤주세요.")
        content=st.text_area("자신이 쓴 글을 입력해주세요.", height=200)

    with container2 :

        st.markdown("#### 중간 피드백")
        if mmid_feedback_btn :
...
with col2 :
    final_feedback_btn = st.button("최종 제출하기", type="primary", use_container_width=True)
